...
def subnational_processor_malta(subnational_dir):
    """
    Customized subnational_stats data processor for Malta files
    ...
    num_states = 1
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Malta'][0] + 1
    ...
    subnation_data = subnation_data.replace({'Malta': 'MALTA'}, inplace=False)
...
class Malta(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_malta, *subnational_dir)
        self.units = units
