...
def subnational_processor_spain(subnational_dir):
    """
    Customized subnational_stats data processor for Spain files
    ...
    num_states = 19
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Spain'][0] + 1
    ...
    subnation_data = subnation_data.replace({
        ...
        'Ciudad de Melilla': 'CEUTA Y MELILLA'
    }, inplace=False)
...
class Spain(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_spain, *subnational_dir)
        self.units = units
