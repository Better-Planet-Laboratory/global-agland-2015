...
def subnational_processor_sweden(subnational_dir):
    """
    Customized subnational_stats data processor for Sweden files
    ...
    num_states = 8
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Sweden'][0] + 1
...
def spatial_map_processor_sweden(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Sweden
    ...
    Returns: (pd) dataframe that contains state level spatial map for Sweden
    """
...
class Sweden(Country):
    STATE_LOOKUP = {
        'Stockholm': ['STOCKHOLM'],
        'Östra Mellansverige': ['OREBRO', 'ÖSTERGÖTLAND', 'SÖDERMANLAND', 'UPPSALA', 'VÄSTMANLAND'],
        'Småland med öarna': ['GOTLAND', 'JÖNKÖPING', 'KALMAR', 'KRONOBERG'],
        'Sydsverige': ['BLEKINGE', 'SKÅNE'],
        'Västsverige': ['VÄSTRA GÖTALAND', 'HALLAND'],
        'Norra Mellansverige': ['DALARNA', 'GÄVLEBORG', 'VÄRMLAND'],
        'Mellersta Norrland': ['JÄMTLAND', 'VÄSTERNORRLAND'],
        'Övre Norrland': ['NORRBOTTEN', 'VÄSTERBOTTEN']
    }
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_sweden, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_sweden(shapefile_dir, Sweden.STATE_LOOKUP)
