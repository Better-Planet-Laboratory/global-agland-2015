...
def subnational_processor_ireland(subnational_dir):
    """
    Customized subnational_stats data processor for Ireland files
    ...
    num_states = 2
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Ireland'][0] + 1
...
def spatial_map_processor_ireland(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Ireland
    ...
    Returns: (pd) dataframe that contains state level spatial map for Ireland
    """
...
class Ireland(Country):
    STATE_LOOKUP = {
        'Border, Midland and Western (NUTS 2013)': ['CAVAN', 'DONEGAL', 'GALWAY', 'LAOIS', 'LEITRIM',
                                                    'LONGFORD', 'LOUTH', 'MAYO', 'MONAGHAN', 'OFFALY',
                                                    'ROSCOMMON', 'SLIGO', 'WESTMEATH'],
        'Southern and Eastern (NUTS 2013)': ['CARLOW', 'CLARE', 'DUBLIN', 'KERRY', 'KILDARE', 'KILKENNY',
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_ireland, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_ireland(shapefile_dir, Ireland.STATE_LOOKUP)
