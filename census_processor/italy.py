...
def subnational_processor_italy(subnational_dir):
    """
    Customized subnational_stats data processor for Italy files
    ...
    num_states = 21
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Italy'][0] + 1
...
class Italy(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_italy, *subnational_dir)
        self.units = units
