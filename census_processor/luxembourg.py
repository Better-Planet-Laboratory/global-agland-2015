...
def subnational_processor_luxembourg(subnational_dir):
    """
    Customized subnational_stats data processor for Luxembourg files
    ...
    num_states = 1
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Luxembourg'][0] + 1
    ...
    subnation_data = subnation_data.replace({'Luxembourg': 'LUXEMBOURG'}, inplace=False)
...
class Luxembourg(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_luxembourg, *subnational_dir)
        self.units = units
...
root = '../'
belgium = Luxembourg(root + 'shapefile/EU/Luxembourg/gadm40_LUX_0.shp',
