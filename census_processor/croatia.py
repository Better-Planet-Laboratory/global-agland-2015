...
def subnational_processor_croatia(subnational_dir):
    """
    Customized subnational_stats data processor for Croatia files
    ...
    num_states = 2
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Croatia'][0] + 1
...
def spatial_map_processor_croatia(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Croatia
    ...
    Returns: (pd) dataframe that contains state level spatial map for Croatia
    """
...
class Croatia(Country):
    # Source:
    # Jadranska Hrvatska: https://hr.wikipedia.org/wiki/Jadranska_Hrvatska
    # Kontinentalna Hrvatska: https://en.wikipedia.org/wiki/Continental_Croatia
    STATE_LOOKUP = {
        'Jadranska Hrvatska': ['DUBROVACKO-NERETVANSKA', 'ISTARSKA', 'LICKO-SENJSKA', 'PRIMORSKO-GORANSKA',
                               'ŠIBENSKO-KNINSKA', 'SPLITSKO-DALMATINSKA', 'ZADARSKA'],
        'Kontinentalna Hrvatska (NUTS 2016)': ['BJELOVARSKA-BILOGORSKA', 'BRODSKO-POSAVSKA', 'GRAD ZAGREB	',
                                               'KARLOVACKA',
                                               'KOPRIVNIČKO-KRIŽEVAČKA', 'KRAPINSKO-ZAGORSKA', 'MEDIMURSKA',
                                               'OSJECKO-BARANJSKA',
                                               'POŽEŠKO-SLAVONSKA', 'SISACKO-MOSLAVACKA', 'VARAŽDINSKA',
                                               'VIROVITICKO-PODRAVSKA',
                                               'VUKOVARSKO-SRIJEMSKA', 'ZAGREBAČKA']
    }
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_croatia, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_croatia(shapefile_dir, Croatia.STATE_LOOKUP)
