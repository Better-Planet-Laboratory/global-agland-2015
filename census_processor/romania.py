...
def subnational_processor_romania(subnational_dir):
    """
    Customized subnational_stats data processor for Romania files
    ...
    num_states = 8
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Romania'][0] + 1
...
def spatial_map_processor_romania(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Romania
    ...
    Returns: (pd) dataframe that contains state level spatial map for Romania
    """
...
class Romania(Country):
    STATE_LOOKUP = {
        'Nord-Vest': ['BIHOR', 'BISTRIȚA-NĂSĂUD', 'CLUJ', 'MARAMUREȘ', 'SĂLAJ', 'SATU MARE'],
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_romania, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_romania(shapefile_dir, Romania.STATE_LOOKUP)
