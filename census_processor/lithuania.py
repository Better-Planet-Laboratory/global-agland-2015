...
def subnational_processor_lithuania(subnational_dir):
    """
    Customized subnational_stats data processor for Lithuania files
    ...
    num_states = 1
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Lithuania'][0] + 1
    ...
    subnation_data = subnation_data.replace({'Lietuva (NUTS 2013)': 'LITHUANIA'}, inplace=False)
...
class Lithuania(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_lithuania, *subnational_dir)
        self.units = units
