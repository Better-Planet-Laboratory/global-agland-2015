...
def subnational_processor_poland(subnational_dir):
    """
    Customized subnational_stats data processor for Poland files
    ...
    num_states = 16
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Poland'][0] + 1
    ...
    subnation_data = subnation_data.replace({
        'Lódzkie (NUTS 2013)': 'ŁÓDZKIE',
        'Mazowieckie (NUTS 2013)': 'MAZOWIECKIE',
        'Malopolskie': 'MAŁOPOLSKIE',
        'Slaskie': 'ŚLĄSKIE',
        'Lubelskie (NUTS 2013)': 'LUBELSKIE',
        'Podkarpackie (NUTS 2013)': 'PODKARPACKIE',
        'Swietokrzyskie (NUTS 2013)': 'ŚWIĘTOKRZYSKIE',
        'Podlaskie (NUTS 2013)': 'PODLASKIE',
...
class Poland(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_poland, *subnational_dir)
        self.units = units
