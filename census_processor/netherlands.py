...
def subnational_processor_netherlands(subnational_dir):
    """
    Customized subnational_stats data processor for Netherlands files
    ...
    num_states = 12
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Netherlands'][0] + 1
...
class Netherlands(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_netherlands, *subnational_dir)
        self.units = units
