...
def subnational_processor_bulgaria(subnational_dir):
    """
    Customized subnational_stats data processor for Bulgaria files
    ...
    num_states = 6
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Bulgaria'][0] + 1
...
def spatial_map_processor_bulgaria(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Bulgaria
    ...
    Returns: (pd) dataframe that contains state level spatial map for Bulgaria
    """
...
class Bulgaria(Country):
    STATE_LOOKUP = {
        'Severozapaden': ['VIDIN', 'MONTANA', 'VRATSA', 'PLEVEN', 'LOVECH'],
        'Severen tsentralen': ['GABROVO', 'VELIKO TARNOVO', 'RAZGRAD', 'RUSE', 'TARGOVISHTE'],
        'Severoiztochen': ['SILISTRA', 'DOBRICH', 'VARNA', 'SHUMEN'],
        'Yugoiztochen': ['BURGAS', 'YAMBOL', 'SLIVEN', 'STARA ZAGORA'],
        'Yugozapaden': ['SOFIA', 'GRAD SOFIYA', 'PERNIK', 'KYUSTENDIL', 'BLAGOEVGRAD'],
        'Yuzhen tsentralen': ['PAZARDZHIK', 'PLOVDIV', 'SMOLYAN', 'KARDZHALI', 'HASKOVO']
    }
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_bulgaria, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_bulgaria(shapefile_dir, Bulgaria.STATE_LOOKUP)
