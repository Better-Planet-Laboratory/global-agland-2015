...
def subnational_processor_portugal(subnational_dir):
    """
    Customized subnational_stats data processor for Portugal files
    ...
    num_states = 7
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Portugal'][0] + 1
...
def spatial_map_processor_portugal(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Portugal
    ...
    Returns: (pd) dataframe that contains state level spatial map for Portugal
    """
...
class Portugal(Country):
    # Source:
    # https://xrei.com/pt/regioes-de-portugal/
    STATE_LOOKUP = {
        'Norte': ['BRAGA', 'BRAGANÇA', 'PORTO', 'VIANA DO CASTELO', 'VILA REAL'],
        'Algarve': ['FARO'],
        'Centro (PT)': ['AVEIRO', 'CASTELO BRANCO', 'COIMBRA', 'GUARDA', 'LEIRIA', 'VISEU'],
        'Área Metropolitana de Lisboa': ['LISBOA', 'SETÚBAL'],
        'Alentejo': ['BEJA', 'ÉVORA', 'PORTALEGRE', 'SANTARÉM'],
        'Região Autónoma dos Açores (PT)': ['AZORES']
    }
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_portugal, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_portugal(shapefile_dir, Portugal.STATE_LOOKUP)
