...
def subnational_processor_austria(subnational_dir):
    """
    Customized subnational_stats data processor for Austria files
    ...
    num_states = 9
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Austria'][0] + 1
...
class Austria(Country):
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_austria, *subnational_dir)
        self.units = units
