...
def subnational_processor_hungary(subnational_dir):
    """
    Customized subnational_stats data processor for Hungary files
    ...
    num_states = 7
    start_index = raw_subnation_data.index[raw_subnation_data['CROPS (Labels)'] == 'Hungary'][0] + 1
...
def spatial_map_processor_hungary(shapefile_dir, state_lookup):
    """
    The state level shapefile could not be found on GADM, so this function helps
    union the corresponding cities into their state in Hungary
    ...
    Returns: (pd) dataframe that contains state level spatial map for Hungary
    """
...
class Hungary(Country):
    STATE_LOOKUP = {
        ...
        'Nyugat-Dunántúl': ['GYOR-MOSON-SOPRON', 'VAS', 'ZALA'],
    ...
    def __init__(self, shapefile_dir, subnational_dir, FAOSTAT_dir, units='Ha'):
        ...
        self.subnational_data = self.get_subnational_data(subnational_processor_hungary, *subnational_dir)
        self.units = units

    def get_spatial_map(self, shapefile_dir):
        spatial_map = spatial_map_processor_hungary(shapefile_dir, Hungary.STATE_LOOKUP)
