...
def pycno(gdf,
    ...
    input gdf is a combination of FAOSTAT and subnational census, therefore not covering
    the whole global regions. Some areas will have "holes". These holes will be replaced 
    by 1s instead of 0s before smoothing 
    ...
    def smooth2D(data):
        ...
        padarray = (apply_along_axis(s1d, 1, padarray) +
                    apply_along_axis(s1d, 0, padarray)) / 2
        # Reinstate nodata
        padarray[mask] = nan
    ...
    while True:
        ...
        # Log all value_array
        if f_start:
            value_array_log = copy(value_array)[None, :, :]
            f_start = False
        else:
            value_array_log = np.concatenate(
                (value_array_log, copy(value_array)[None, :, :]), axis=0)
    ...
    return (value_array, trans, gdf.crs, value_array_log)
